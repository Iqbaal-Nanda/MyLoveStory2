...
// ----------------------------------------------------
// LOCAL STORAGE & FILE SYSTEM ACCESS API HELPERS
// ----------------------------------------------------
const DB_NAME = 'MyLoveStoryFS';
const STORE_NAME = 'handles';
let localImagesDirHandle = null;
...
async function getSavedDirHandle() {
    try {
        ...
        return new Promise((resolve, reject) => {
            ...
            const request = store.get('imagesDir');
...
async function saveDirHandle(handle) {
    try {
        ...
        return new Promise((resolve, reject) => {
            ...
            const store = transaction.objectStore(STORE_NAME);
            const request = store.put(handle, 'imagesDir');
...
async function getImagesDirectoryHandle(promptUser = true) {
    if (localImagesDirHandle) {
        if (await verifyPermission(localImagesDirHandle, true)) {
            return localImagesDirHandle;
        }
    }
    ...
    if (promptUser) {
        try {
            ...
            alert("Silakan hubungkan folder 'images' dari direktori proyek MyLoveStory Anda agar gambar dapat disimpan secara lokal.");
            const handle = await window.showDirectoryPicker({ mode: 'readwrite' });
            
            if (handle.name !== 'images') {
                const confirmCorrect = confirm(`Anda memilih folder '${handle.name}'. Apakah Anda yakin ini adalah folder 'images' di dalam folder proyek Anda?`);
                if (!confirmCorrect) return null;
            }
            
            localImagesDirHandle = handle;
            await saveDirHandle(handle);
            updateLocalStorageStatusUI(true, handle.name);
            return localImagesDirHandle;
        } catch (e) {
...
async function saveFileToImagesDir(file) {
    const dirHandle = await getImagesDirectoryHandle(true);
    if (!dirHandle) {
        throw new Error("Folder 'images' tidak terhubung atau akses ditolak.");
    }
...
window.updateLocalStorageStatusUI = function (isConnected, folderName = "", isStale = false, isNotSupported = false) {
    const statusEls = document.querySelectorAll('.local-storage-status');
    const btnEls = document.querySelectorAll('.btn-connect-local');
    const infoEls = document.querySelectorAll('.local-storage-info');
    
    statusEls.forEach((statusEl, idx) => {
        const btnEl = btnEls[idx];
        const infoEl = infoEls[idx];
        
        if (isNotSupported) {
            statusEl.textContent = "Hanya via Live Server";
            statusEl.style.color = "rgba(255,255,255,0.5)";
            if (btnEl) btnEl.style.display = 'none';
            if (infoEl) infoEl.innerHTML = '<i class="fa-solid fa-circle-info"></i> API Penyimpanan Lokal tidak didukung di protokol <code>file://</code>. Buka proyek via Live Server di VS Code untuk mengaktifkan penyimpanan langsung ke folder <code>images/</code>. Upload saat ini menggunakan cloud ImgBB.';
        } else if (isConnected) {
            statusEl.textContent = `Terhubung (${folderName})`;
            statusEl.style.color = "#00e676";
            if (btnEl) {
                btnEl.textContent = "Putuskan Folder / Ganti Folder";
                btnEl.style.background = "rgba(255,255,255,0.1)";
                btnEl.style.display = 'block';
            }
            if (infoEl) infoEl.innerHTML = '<i class="fa-solid fa-circle-check" style="color:#00e676;"></i> Upload foto akan langsung disimpan di folder lokal Anda <code>images/</code>.';
        } else if (isStale) {
            statusEl.textContent = "Butuh Otorisasi";
            statusEl.style.color = "#f77f00";
            if (btnEl) {
                btnEl.textContent = "Otorisasi Ulang Folder";
                btnEl.style.background = "var(--primary-color)";
                btnEl.style.display = 'block';
            }
            if (infoEl) infoEl.innerHTML = '<i class="fa-solid fa-circle-exclamation" style="color:#f77f00;"></i> Browser meminta Anda mengizinkan kembali akses folder images lokal Anda.';
        } else {
            statusEl.textContent = "Belum Terhubung";
            statusEl.style.color = "#ff4b72";
            if (btnEl) {
                btnEl.textContent = "Hubungkan Folder 'images' Lokal";
                btnEl.style.background = "var(--primary-color)";
                btnEl.style.display = 'block';
            }
            if (infoEl) infoEl.innerHTML = '<i class="fa-solid fa-circle-info"></i> Hubungkan folder <code>images/</code> agar foto tersimpan secara fisik di disk Anda. Jika tidak terhubung, foto akan diupload ke cloud ImgBB.';
        }
    });
};

window.connectLocalFolder = async function() {
    const db = await getDB();
    const transaction = db.transaction(STORE_NAME, 'readonly');
    const store = transaction.objectStore(STORE_NAME);
    const existing = await new Promise((resolve) => {
        const req = store.get('imagesDir');
        req.onsuccess = () => resolve(req.result);
    });

    if (existing || localImagesDirHandle) {
        localImagesDirHandle = null;
        try {
            const db = await getDB();
            const writeTrans = db.transaction(STORE_NAME, 'readwrite');
            const writeStore = writeTrans.objectStore(STORE_NAME);
            await new Promise((resolve, reject) => {
                const req = writeStore.delete('imagesDir');
                req.onsuccess = resolve;
                req.onerror = reject;
            });
            updateLocalStorageStatusUI(false);
            showToast("Penyimpanan lokal diputuskan.", "success");
        } catch (e) {
            console.error(e);
        }
        return;
    ...
    try {
        const handle = await getImagesDirectoryHandle(true);
        if (handle) {
            updateLocalStorageStatusUI(true, handle.name);
            showToast(`Terhubung ke folder: ${handle.name}!`, "success");
        } else {
            updateLocalStorageStatusUI(false);
        }
    } catch (e) {
        console.error(e);
        showToast("Gagal menghubungkan folder.", "error");
    }
};

async function checkLocalFolderOnLoad() {
    if (window.showDirectoryPicker) {
        const saved = await getSavedDirHandle();
        if (saved) {
            ...
            try {
                ...
                if (permission === 'granted') {
                    localImagesDirHandle = saved;
                    updateLocalStorageStatusUI(true, saved.name);
                } else {
                    updateLocalStorageStatusUI(false, saved.name, true);
                }
            } catch (e) {
                console.error("Error querying permission on load:", e);
                updateLocalStorageStatusUI(false);
            }
        } else {
            updateLocalStorageStatusUI(false);
        ...
        updateLocalStorageStatusUI(false, "", false, true);
    }
}

function getDisplayUrl(url, width = null) {
    if (!url) return '';
...
if (uploadForm) {
    uploadForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (dirHandle) {
                submitBtn.textContent = "Menyimpan ke Lokal...";
                imageUrl = await saveFileToImagesDir(fileInput.files[0]);
            } else {
            ...
            console.error(error);
            showToast('Terjadi kesalahan saat menyimpan gambar.', 'error');
        } finally {
...
function loadGallery() {
    ...
    items.forEach((item, index) => {
        ...
        const proxyUrl = getDisplayUrl(item.image, 600);
...
window.loadProfiles = function () {
    ...
    if (urlIqbaal) {
        imgA.src = getDisplayUrl(urlIqbaal, 400);
    ...
    if (urlSilvi) {
        imgS.src = getDisplayUrl(urlSilvi, 400);
...
window.uploadProfile = async function (input, person) {
    ...
    try {
        ...
        if (dirHandle) {
            imageUrl = await saveFileToImagesDir(input.files[0]);
        } else {
            const formData = new FormData();
        ...
        if (imageUrl) {
            localStorage.setItem(`cinta_profile_${person}`, imageUrl);
            loadProfiles();
            showToast("Foto profil berhasil diubah!");
        }
    } catch (error) {
        showToast("Terjadi kesalahan saat mengubah profil.", 'error');
        imgElement.src = originalSrc;
    }
};
...
window.renderNovel = function () {
    ...
    novelData.forEach((item, i) => {
        const proxyUrl = getDisplayUrl(item.image);
    ...
    btnSubmitNovel.onclick = async () => {
        ...
        try {
            ...
            if (fileInput.files[0]) {
                ...
                if (dirHandle) {
                    btnSubmitNovel.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Menyimpan ke Lokal...';
                    imageUrl = await saveFileToImagesDir(fileInput.files[0]);
                } else {
                    btnSubmitNovel.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Mengupload ke Cloud...';
                    const formData = new FormData();
                    formData.append("image", fileInput.files[0]);
                    ...
                    if (data.success) {
                        ...
                        throw new Error("Upload gagal: " + (data.error ? data.error.message : ""));
                    }
                }
            }
...
window.loadKoleksiGaleri = function () {
    ...
    items.forEach((item, index) => {
        ...
        const proxyUrl = getDisplayUrl(item.image, 400);
